...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->btnOpen->Location = System::Drawing::Point(349, 12);
			...
			this->ClientSize = System::Drawing::Size(436, 328);
...
#pragma endregion
	private: 
		float left = 30, right = 100, top = 20, bottom = 50; // расстояния до границ окна
	...
	private: System::Void MyForm_KeyDown(System::Object^ sender, System::Windows::Forms::KeyEventArgs^ e) {
		...
		float Wcx = left+Wx/2; // координаты центра
		float Wcy = top+Wy/2; // текущего окна
		...
		case Keys::E:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = rotate(-0.01f) * T; // поворот на 0.01 радиан относительно нового центра (по часовой)
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
		...
		case Keys::R:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = rotate(-0.05f) * T; // поворот на 0.05 радиан относительно нового центра (по часовой)
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
		...
		case Keys::Y:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = rotate(0.05f) * T; // поворот на 0.05 радиан относительно нового центра (против часовой)
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
		...
		case Keys::U:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = mirrorX() * T; // зеркальное отражение относительно горизонтальной
			//оси, проходящей через центр окна, соответственно;
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
		...
		case Keys::J:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = mirrorY() * T; // зеркальное отражение относительно вертикальной
			//оси, проходящей через центр окна, соответственно;
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
